...
// Persistência em disco (as propostas sobrevivem a restarts do PM2)
import { promises as fsp } from "fs";
const PROPOSALS_FILE = path.join(os.tmpdir(), "devai-proposals.json");
async function loadProposalsDisk() {
    try {
        const raw = await fsp.readFile(PROPOSALS_FILE, "utf-8");
        const arr = JSON.parse(raw);
        arr.forEach((p: any) => pendingProposals.set(p.id, p));
    }
    catch (e: any) {
        /* primeira execução */
    }
...
async function saveProposalsDisk() {
    try {
        ...
        console.warn("[SelfImprove] save disk:", (e as Error).message);
    }
}
loadProposalsDisk();
// ─── Shell Helper ───
...
// ─── Run Single Test ───
function runSingleTest(cwd: any) {
...
// ─── Run Tests 20 Times ───
function runTests20Times(cwd: any) {
...
// ─── Analyze Failure ───
function analyzeFailure(results: any) {
...
export async function executeApprovedImprovement(proposalId: any, changes: any) {
...
// ─── Commit and Push ───
async function commitAndPush(cwd: any, appliedFiles: any, proposal: any) {
...
// ─── Apply Changes ───
function applyChanges(cwd: any, changes: any) {
    const applied = [];
 ...
 */
export async function createImprovementProposal(title: any, description: any, filesToChange: any, risks: any, benefits: any, estimatedTime: any) {
    const proposal = {
        id: generateId(),
        title,
        description,
 ...
 */
export function rejectProposal(proposalId: any) {
    const proposal = pendingProposals.get(proposalId);
    if (!proposal)
        return null;
 ...
 */
export function getProposal(proposalId: any) {
...
export async function analyzeForImprovements() {
    ...
    try {
        ...
        if (tsCheck.exitCode !== 0) {
            const proposal = await createImprovementProposal("Corrigir Erros TypeScript", `Erros de TypeScript detectados: ${tsCheck.stderr.slice(0, 300)}`, [], ["Pode exigir refatoração de tipos"], ["Build limpo", "Sem erros no deploy"], "15-30 minutos");
            improvements.push(proposal);
        }
        return improvements;
    }
...
export function executeSystemCommand(command: string, cwd: string, timeout: number = 30000) {
